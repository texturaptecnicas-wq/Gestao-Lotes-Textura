      ...
      <div className="min-h-screen p-4 md:p-8">
        <div className="max-w-7xl mx-auto">
          <motion.header initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            ...
            <div className="glass-effect rounded-xl p-1 flex items-center gap-1">
              {filterOptions.map((status) => (
                <motion.button key={status.id} layout onClick={() => handleFilterChange(status.id)} className={`relative w-full px-3 py-3 rounded-lg font-semibold whitespace-nowrap transition-colors flex items-center justify-center gap-2 text-sm ${filterStatus === status.id ? '' : 'text-slate-300 hover:text-white'}`}>
            ...
            {filterStatus === 'programado' && (
              <div className="mt-4 glass-effect rounded-xl p-1 flex items-center gap-1 flex-wrap">
                {cabineFilterOptions.map(cab => (
                   <motion.button key={cab} layout onClick={() => setCabineFilter(cab)} className={`relative flex-1 px-3 py-2 rounded-lg font-semibold transition-colors text-sm ${cabineFilter === cab ? '' : 'text-slate-300 hover:text-white'}`}>
                    {cabineFilter === cab && <motion.div layoutId="active-cabine-pill" className="absolute inset-0 bg-slate-700/80 rounded-lg z-0" />}
                    <span className="relative z-10">{cab === 'all' ? 'Todas Cabines' : `Cabine ${cab}`}</span>
                   </motion.button>
                ))}
              </div>
            )}
